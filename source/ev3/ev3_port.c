...
/*  ev3_port.c was generated by yup.py (yupp) 0.7b7
    out of ev3_port.yu-c at 2015-02-01 14:52
 *//**
...
uint8_t get_port_type_inx( uint8_t sn )
{
	...
	return ( PORT_TYPE__COUNT_ );
}

uint8_t get_port_port_inx( uint8_t sn, uint8_t *extport )
{
	char buf[ 32 ];
...
int ev3_port_init( void )
{
	...
	while ( p ) {
		if (( ev3_string_suffix( "port", &p, &sn ) == 1 ) && ( sn < PORT_DESC__LIMIT_)) {
			...
			ev3_port[ sn ].type_inx = get_port_type_inx( sn );
			ev3_port[ sn ].port = get_port_port_inx( sn, &ev3_port[ sn ].extport );
			++cnt;
		}
		p = strtok( NULL, " " );
	}
...
static uint8_t ht_nxt_smux_port_inx( const char *name )
{
...
static uint8_t legoev3_input_port_inx( const char *name )
{
...
static uint8_t legoev3_output_port_inx( const char *name )
{
...
static uint8_t ms_ev3_smux_port_inx( const char *name )
{
...
static uint8_t wedo_port_inx( const char *name )
{
...
{
	/* FIXME: We need check a type of the port to distinguish ports with same prefixes.
	 */
	...
	*extport = EXT_PORT__NONE_;
	...
	if ( strlen( name ) == HT_NXT_SMUX_PORT_FULL_LEN ) {
		port = legoev3_input_port_inx( name );
		if ( port != EV3_PORT__NONE_ ) {
			*extport = ht_nxt_smux_port_inx( name + HT_NXT_SMUX_PORT_PARENT_LEN + 1 );
	...
	if ( strlen( name ) == LEGOEV3_OUTPUT_PORT_FULL_LEN ) {
		port = legoev3_output_port_inx( name );
		if ( port != EV3_PORT__NONE_ ) {
			return ( port );
	...
	if ( strlen( name ) == MS_EV3_SMUX_PORT_FULL_LEN ) {
		port = legoev3_input_port_inx( name );
		if ( port != EV3_PORT__NONE_ ) {
			*extport = ms_ev3_smux_port_inx( name + MS_EV3_SMUX_PORT_PARENT_LEN + 1 );
			if ( *extport != EXT_PORT__NONE_ ) {
				return ( port );
			}
	...
	if ( strlen( name ) == WEDO_PORT_FULL_LEN ) {
		port = legoev3_input_port_inx( name );
		if ( port != EV3_PORT__NONE_ ) {
			*extport = wedo_port_inx( name + WEDO_PORT_PARENT_LEN + 1 );
			if ( *extport != EXT_PORT__NONE_ ) {
				return ( port );
			}
