...
/*  ev3_tacho.c was generated by yup.py (yupp) 0.7b4
    out of ev3_tacho.yu-c at 2014-09-02 18:25
 *//**
...
char *get_tacho_type_id_string( uint8_t type_id )
{
	switch ( type_id ) {
	case 1:
		return "TACHO";
...
int search_tacho_port( uint8_t type_id )
{
	int i;

	for ( i = 0; i < OUT__COUNT_; i++ ) {
		if ( ev3_tacho[ i ].detected && ( ev3_tacho[ i ].type_id == type_id )) return ( i );
	}
	return ( EOF );
}

static bool get_string_suffix( const char *prefix, char *s, uint32_t *buf )
{
	char *end;
...
int ev3_tacho_init( void )
{
	...
	p = strtok( list, " " );
	while ( p ) {
		if ( get_string_suffix( "tacho-motor", p, &id )) {
			if ( get_tacho_port_name( id, s, sizeof( s ))) {
				if ( strcmp( s, "outA") == 0 ) port = OUTA;
				else
				if ( strcmp( s, "outB") == 0 ) port = OUTB;
				else
				if ( strcmp( s, "outC") == 0 ) port = OUTC;
				else
				if ( strcmp( s, "outD") == 0 ) port = OUTD;
				else

					port = EOF;

				if ( port != EOF ) {
					ev3_tacho[ port ].detected = true;
