...
/*  ev3_tacho.c was generated by yup.py (yupp) 0.7b7
    out of ev3_tacho.yu-c at 2014-11-26 12:58
 *//**
...
uint8_t get_tacho_type_inx( uint8_t sn )
{
	...
	if ( strcmp( t, "tacho" ) == 0 ) return TACHO;
	if ( strcmp( t, "minitacho" ) == 0 ) return MINITACHO;

	return ( TACHO_TYPE__COUNT_ );
}

uint8_t get_tacho_port_inx( uint8_t sn, uint8_t *extport )
{
	char buf[ 32 ];
...
int ev3_tacho_init( void )
{
	...
	while ( p ) {
		if (( ev3_string_suffix( "motor", &p, &sn ) == 1 ) && ( sn < TACHO_DESC__LIMIT_)) {
			...
			ev3_tacho[ sn ].type_inx = get_tacho_type_inx( sn );
			ev3_tacho[ sn ].port = get_tacho_port_inx( sn, &ev3_tacho[ sn ].extport );
			++cnt;
		}
		p = strtok( NULL, " " );
	}
