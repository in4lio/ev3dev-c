...
/*  ev3_sensor.c was generated by yup.py (yupp) 0.7b8
    out of ev3_sensor.yu-c at 2015-02-08 12:55
 *//**
...
const char *ev3_sensor_mode( uint8_t mode_inx )
{
	...
	case HT_NXT_COMPASS_COMAPASS:
		return "COMAPASS";

	case HT_NXT_MAG_MAG:
		return "MAG";

	case HT_NXT_IR_SEEKER_V2_DC:
		return "DC";
	case HT_NXT_IR_SEEKER_V2_AC:
		return "AC";
	case HT_NXT_IR_SEEKER_V2_DC_ALL:
		return "DC-ALL";
	case HT_NXT_IR_SEEKER_V2_AC_ALL:
		return "AC-ALL";
	...
	case EV3_UART_30_US_DIST_CM:
		return "US-DIST-CM";
	case EV3_UART_30_US_DIST_IN:
		return "US-DIST-IN";
	case EV3_UART_30_US_LISTEN:
		return "US-LISTEN";
	case EV3_UART_30_US_SI_CM:
		return "US-SI-CM";
	case EV3_UART_30_US_SI_IN:
		return "US-SI-IN";
	case EV3_UART_30_US_DC_CM:
		return "US-DC-CM";
	case EV3_UART_30_US_DC_IN:
		return "US-DC-IN";

	case EV3_UART_32_GYRO_ANG:
		return "GYRO-ANG";
	case EV3_UART_32_GYRO_RATE:
		return "GYRO-RATE";
	case EV3_UART_32_GYRO_FAS:
		return " GYRO-FAS";
	case EV3_UART_32_GYRO_G_AND_A:
		return "GYRO-G&A";
	case EV3_UART_32_GYRO_CAL:
		return "GYRO-CAL";

	case EV3_UART_29_COL_REFLECT:
		return "COL-REFLECT";
	case EV3_UART_29_COL_AMBIENT:
		return "COL-AMBIENT";
	case EV3_UART_29_COL_COLOR:
		return "COL-COLOR";
	case EV3_UART_29_REF_RAW:
		return "REF-RAW";
	case EV3_UART_29_RGB_RAW:
		return "RGB-RAW";
	case EV3_UART_29_COL_CAL:
		return "COL-CAL";

	case LEGO_EV3_TOUCH_TOUCH:
		return "TOUCH";

	case EV3_UART_33_IR_PROX:
		return "IR-PROX";
	case EV3_UART_33_IR_SEEK:
		return "IR-SEEK";
	case EV3_UART_33_IR_REMOTE:
		return "IR-REMOTE";
	case EV3_UART_33_IR_REM_A:
		return "IR-REM-A";
	case EV3_UART_33_IR_S_ALT:
		return "IR-S-ALT";
	case EV3_UART_33_IR_CAL:
		return "IR-CAL";
...
uint8_t get_sensor_mode_inx( uint8_t sn, uint8_t type_inx )
{
	...
	case HT_NXT_COMPASS:
		if ( strcmp( buf, "COMAPASS" ) == 0 ) return HT_NXT_COMPASS_COMAPASS;
	...
	case HT_NXT_IR_SEEKER_V2:
		if ( strcmp( buf, "DC" ) == 0 ) return HT_NXT_IR_SEEKER_V2_DC;
		if ( strcmp( buf, "AC" ) == 0 ) return HT_NXT_IR_SEEKER_V2_AC;
		if ( strcmp( buf, "DC-ALL" ) == 0 ) return HT_NXT_IR_SEEKER_V2_DC_ALL;
		if ( strcmp( buf, "AC-ALL" ) == 0 ) return HT_NXT_IR_SEEKER_V2_AC_ALL;
	...
	case EV3_UART_30:
		if ( strcmp( buf, "US-DIST-CM" ) == 0 ) return EV3_UART_30_US_DIST_CM;
		if ( strcmp( buf, "US-DIST-IN" ) == 0 ) return EV3_UART_30_US_DIST_IN;
		if ( strcmp( buf, "US-LISTEN" ) == 0 ) return EV3_UART_30_US_LISTEN;
		if ( strcmp( buf, "US-SI-CM" ) == 0 ) return EV3_UART_30_US_SI_CM;
		if ( strcmp( buf, "US-SI-IN" ) == 0 ) return EV3_UART_30_US_SI_IN;
		if ( strcmp( buf, "US-DC-CM" ) == 0 ) return EV3_UART_30_US_DC_CM;
		if ( strcmp( buf, "US-DC-IN" ) == 0 ) return EV3_UART_30_US_DC_IN;

		break;

	case EV3_UART_32:
		if ( strcmp( buf, "GYRO-ANG" ) == 0 ) return EV3_UART_32_GYRO_ANG;
		if ( strcmp( buf, "GYRO-RATE" ) == 0 ) return EV3_UART_32_GYRO_RATE;
		if ( strcmp( buf, " GYRO-FAS" ) == 0 ) return EV3_UART_32_GYRO_FAS;
		if ( strcmp( buf, "GYRO-G&A" ) == 0 ) return EV3_UART_32_GYRO_G_AND_A;
		if ( strcmp( buf, "GYRO-CAL" ) == 0 ) return EV3_UART_32_GYRO_CAL;

		break;

	case EV3_UART_29:
		if ( strcmp( buf, "COL-REFLECT" ) == 0 ) return EV3_UART_29_COL_REFLECT;
		if ( strcmp( buf, "COL-AMBIENT" ) == 0 ) return EV3_UART_29_COL_AMBIENT;
		if ( strcmp( buf, "COL-COLOR" ) == 0 ) return EV3_UART_29_COL_COLOR;
		if ( strcmp( buf, "REF-RAW" ) == 0 ) return EV3_UART_29_REF_RAW;
		if ( strcmp( buf, "RGB-RAW" ) == 0 ) return EV3_UART_29_RGB_RAW;
		if ( strcmp( buf, "COL-CAL" ) == 0 ) return EV3_UART_29_COL_CAL;
	...
	case EV3_UART_33:
		if ( strcmp( buf, "IR-PROX" ) == 0 ) return EV3_UART_33_IR_PROX;
		if ( strcmp( buf, "IR-SEEK" ) == 0 ) return EV3_UART_33_IR_SEEK;
		if ( strcmp( buf, "IR-REMOTE" ) == 0 ) return EV3_UART_33_IR_REMOTE;
		if ( strcmp( buf, "IR-REM-A" ) == 0 ) return EV3_UART_33_IR_REM_A;
		if ( strcmp( buf, "IR-S-ALT" ) == 0 ) return EV3_UART_33_IR_S_ALT;
		if ( strcmp( buf, "IR-CAL" ) == 0 ) return EV3_UART_33_IR_CAL;
