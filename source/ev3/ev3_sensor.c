...
/*  ev3_sensor.c was generated by yup.py (yupp) 0.7b6
    out of ev3_sensor.yu-c at 2014-10-01 15:21
 *//**
...
bool ev3_get_sensor_connected( uint8_t port )
{
	return ( ev3_sensor[( port < INPUT__COUNT_ ) ? port : 0 ].connected );
}

uint32_t ev3_get_sensor_id( uint8_t port )
{
	return ( ev3_sensor[( port < INPUT__COUNT_ ) ? port : 0 ].id );
}

uint32_t ev3_get_sensor_type_id( uint8_t port )
{
	return ( ev3_sensor[( port < INPUT__COUNT_ ) ? port : 0 ].type_id );
}
