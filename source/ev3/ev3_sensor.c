...
/*  ev3_sensor.c was generated by yup.py (yupp) 0.7b6
    out of ev3_sensor.yu-c at 2014-11-07 18:10
 *//**
...
uint8_t get_sensor_type_inx( uint8_t sn )
{
	...
	if ( strcmp( buf, "nxt-i2c" ) == 0 ) return NXT_I2C;
	...
	if ( strcmp( buf, "tmp275" ) == 0 ) return TMP275;
	...
	if ( strcmp( buf, "lego-nxt-ultrasonic" ) == 0 ) return LEGO_NXT_ULTRASONIC;
	if ( strcmp( buf, "ms-light-array" ) == 0 ) return MS_LIGHT_ARRAY;
	if ( strcmp( buf, "ms-8ch-servo" ) == 0 ) return MS_8CH_SERVO;
	if ( strcmp( buf, "pcf8574" ) == 0 ) return PCF8574;
	if ( strcmp( buf, "pcf8591" ) == 0 ) return PCF8591;
	if ( strcmp( buf, "ds1307" ) == 0 ) return DS1307;
