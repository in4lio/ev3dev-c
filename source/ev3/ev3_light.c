...
/*  ev3_light.c was generated by yup.py (yupp) 0.7b4
    out of ev3_light.yu-c at 2014-09-02 18:24
 *//**
...
static const uint8_t __LED_GREEN[ LIT__LOC__ ] = { EV3_GREEN_LEFT, EV3_GREEN_RIGHT };
static const uint8_t __LED_RED[ LIT__LOC__ ] = { EV3_RED_LEFT, EV3_RED_RIGHT };
static const uint8_t __LED_GREEN_STATE  [ LIT__COL__ ] = { 0, 1, 0, 1 };
static const uint8_t __LED_RED_STATE    [ LIT__COL__ ] = { 0, 0, 1, 1 };
...
void set_light_trigger( uint8_t loc, uint8_t col, uint8_t trigger )
{
	...
	if ( __LED_GREEN_TRIGGER[ col ]) set_led_trigger_index( __LED_GREEN[ loc ], trigger );
	if ( __LED_RED_TRIGGER[ col ]) set_led_trigger_index( __LED_RED[ loc ], trigger );
}
...
uint8_t get_light_trigger( uint8_t loc, uint8_t col )
{
	...
	if ( __LED_GREEN_TRIGGER[ col ]) {
		green = get_led_trigger_index( __LED_GREEN[ loc ]);
		if ( !__LED_RED_TRIGGER[ col ]) return ( green );
	}
	if ( __LED_RED_TRIGGER[ col ]) {
		red = get_led_trigger_index( __LED_RED[ loc ]);
		if ( !__LED_GREEN_TRIGGER[ col ]) return ( red );
	}
	return (( green == red ) ? green : TRIGGER__COUNT_ );
}

bool set_light_blink( uint8_t loc, uint8_t col, int delay_on, int delay_off )
{
